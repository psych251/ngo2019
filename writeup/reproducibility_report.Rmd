...
data_files <- list.files(file.path("data/pilot_b"), pattern = "*.csv", full.names = TRUE)
data_files
...
all_data_cleaned <- all_data |>
  ...
  select(subject_id, study_id, session_id,
         trial_index, time_elapsed, rt, stimulus, task,
...
compute_data_model <- function(group_one, group_two, num_events = 24) {
  ...
  test_set <- all_data_cleaned |>
    ...
    select(id, retrieval_group, accuracy) |> # select only the necessary columns
    filter(retrieval_group == group_one | retrieval_group == group_two) |> # filter out relevant groups from function params
    pivot_wider(id_cols = id, names_from = retrieval_group, values_from = accuracy) |> # make long data frame wide for data dependent calculations
    rowwise() |>
  ...
  prop_all_correct <- sum(test_set$sum == 2) / num_events
  
  # get proportion of all incorrect
  prop_all_incorrect <- sum(test_set$sum == 0) / num_events
  
  return(prop_all_correct + prop_all_incorrect)
}
